    ...
    <template v-else>
      <td class="d-print-non" v-if="detailed && !show_shopping" @click="done">
        <i
          ...
          v-if="ingredient.checked"
        ></i>
        <i
          class="far fa-check-circle text-primary"
          v-if="!ingredient.checked"
        ></i>
      </td>
      <td class="text-nowrap" @click="done">
        <span v-if="ingredient.amount !== 0">
          {{ calculateAmount(ingredient.amount) }}
        </span>
        <span v-if="ingredient.unit !== null && !ingredient.no_amount">
      ...
      <td @click="done">
        <template v-if="ingredient.food !== null">
          ...
          >
          <span v-if="ingredient.food.recipe === null">{{
      ...
      <td v-if="detailed && !show_shopping">
        <div v-if="ingredient.note">
          <span
            v-b-popover.hover="ingredient.note"
            class="d-print-none touchable"
          >
            <i class="far fa-comment"></i>
          </span>

          <div class="d-none d-print-block">
            <i class="far fa-comment-alt d-print-none"></i>
            {{ ingredient.note }}
          </div>
        </div>
      </td>
      <td v-else-if="show_shopping" class="text-right text-nowrap">
        ...
        <b-button
          ...
          "
          variant="link"
          v-b-popover.hover.click.blur.html.top="{
            title: ShoppingPopover,
  ...
  computed: {
    ...
    ShoppingPopover() {
      if (this.ingredient?.shopping_status == false) {
        ...
        let category =
          this.$t("Category") + ": " + this.ingredient?.category ??
          this.$t("Undefined");
        let popover = [];
        ...
        return (
          "<table class='table-small'><th colspan='4'>" +
          category +
          "</th>" +
          popover.join("") +
          "</table>"
