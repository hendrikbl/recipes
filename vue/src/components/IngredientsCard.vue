    ...
    <div :class="{ 'card-body': header }">
      ...
      >
        <div class="col col-md-6 offset-md-6 text-right">
          <b-form-select
            v-model="selected_shoppingrecipe"
            :options="ShoppingRecipes"
            size="sm"
          />
        </div>
      </div>
        ...
        <div class="col-md-12">
          <table class="table table-sm">
            <template v-for="s in steps">
              <tr
                :key="s.id"
                v-if="s.show_as_header && s.name !== '' && !add_shopping_mode"
              >
                <td colspan="5">
                  <b>{{ s.name }}</b>
                </td>
...
export default {
  name: "IngredientCard",
  mixins: [ApiMixin],
  components: { IngredientComponent, CustomInputSpinButton },
  props: {
    steps: {
      type: Array,
      default() {
  ...
  computed: {
    ShoppingRecipes() {
      ...
      let recipe_in_list = this.shopping_list
        ...
        .filter((x) => x?.recipe == this.recipe.id);
      return [...new Map(recipe_in_list.map((x) => [x["value"], x])).values()]; //  filter to unique lists
    },
  },
  watch: {
    servings: function (newVal) {
      this.local_servings = newVal;
    },
    local_servings: function (newVal) {
      this.$emit("change-servings", newVal);
    },
    ...
    selected_shoppingrecipe: function (newVal, oldVal) {
      this.update_shopping = this.shopping_list
        .filter((x) => x.list_recipe === newVal)
        .map((x) => x.ingredient);
      this.$emit(
        "change-servings",
        this.ShoppingRecipes.filter(
          (x) => x.value === this.selected_shoppingrecipe
        )[0].servings
      );
    },
  },
  mounted() {
    ...
    this.local_servings = this.servings;
  },
  methods: {
          ...
          if (this.add_shopping_mode) {
            if (this.recipe_list) {
              this.$emit(
                "starting-cart",
                this.shopping_list
                  .filter((x) => x.list_recipe === this.recipe_list)
                  .map((x) => x.ingredient)
              );
            } else {
              this.$emit(
                ...
                this.steps
                  ...
                  .filter(
                    (x) =>
                      x?.food?.food_onhand == false &&
                      x?.food?.ignore_shopping == false
                  )
                  .map((x) => x.id)
              );
            }
    ...
    addShopping: function (e) {
      ...
      if (e.add) {
        ...
        this.update_shopping = [
          ...this.update_shopping.filter((x) => x !== e.item.id),
        ];
        this.shopping_list = [
          ...this.shopping_list.filter(
            (x) =>
              !(
                x.ingredient === e.item.id &&
                x.list_recipe === this.selected_shoppingrecipe
              )
          ),
        ];
      }
    ...
    prepareIngredient: function (i) {
      ...
      let selected_list = this.shopping_list.filter(
        (x) =>
          x.list_recipe === this.selected_shoppingrecipe &&
          x.ingredient === i.id
      );
      ...
      let checked =
        ...
        (this.add_shopping_mode &&
          !this.selected_shoppingrecipe &&
          !i?.food?.ignore_recipe &&
          !i?.food?.food_onhand);
